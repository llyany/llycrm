...
String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/";
	...
	//准备前面正常阶段，和后面丢失阶段的分界点下标
	int point = 0;
	for(int i=0;i<dvList.size();i++){

		DicValue dv = dvList.get(i);//获得DicValue实体

		String stage = dv.getValue();//获得实体的名称属性值
		String possibility = pMap.get(stage);

		if("0".equals(possibility)){

			point = i;//这里是获得第七阶段的下标
...
<script type="text/javascript">
	...
	function changeStage(listStage,i) {
		...
		$.ajax({
			...
			data : {
				...
				"stage" : listStage,	//变更交易用 生成交易历史用
				"id" : "${t.id}",	//变更交易用 生成交易历史用，tranId是tbl_tran_history外键
	...
	function changeIcon(listStage,index1) {
		...
		//当前阶段下标
		var index = index1;
		//前面正常阶段和后面丢失阶段的分界点下标 0 1 2 3 4 5 6 7 8
		var point = "<%=point%>";
		...
		//如果当前阶段可能性为0，前面7个一定是黑圈，后两个一个是红叉，一个是黑叉
		...
		//如果当前阶段可能性不为0	，前面7个有可能像是绿圈，绿色标记，黑圈，后两个一定是黑叉
		}else{

			//取得前7个阶段，point是最开始从application域中得到的第七阶段的下标
			for(var i=0;i<point;i++){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段的可能性为0,说明当前阶段是后两个阶段
			/*
			由定义属性可知：
			08\u4E22\u5931\u7684\u7EBF\u7D22=0
			09\u56E0\u7ADE\u4E89\u4E22\u5931\u5173\u95ED=0*/
			//前7个阶段一定是黑圈，后两个一个是红叉，一个是黑叉，情况已定，剩下是实现！！！！
			if("0".equals(currentPossibility)){

				//遍历数据字典值列表，分别取出9个阶段及对应的可能性进一步判断，通过实际情况来确定其显示的标签是什么
				for(int i=0;i<dvList.size();i++){
					...
					//判断每一个遍历出来的阶段的可能性
					//下面通过比较阶段名是否相同来判断，因为有的阶段可能性可能相同
					//总之，就是不同阶段显示标签的问题
					//如果遍历出来的可能性为0，说明是后两个阶段，一个是红叉，一个是黑叉
		...
		//如果当前阶段的可能性不为0，说明当前阶段是前7个阶段
		//前7个阶段有可能是绿圈，绿色标记，黑圈，后俩个一定是黑叉
		}else{
