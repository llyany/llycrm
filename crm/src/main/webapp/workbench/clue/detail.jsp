...
<script type="text/javascript">
	...
	$(function(){
		...
		//为搜索市场活动的文本框绑定事件，根据关键字查询并展现所需市场活动列表
			...
			if(event.keyCode==13){
				...
				$.ajax({
					...
					success : function (data) {
						...
						$("#activitySearchBody").html(html);//把信息铺到指定div上
				...
				//以上市场活动展现完毕后，需要及时终止该方法，否则会触发模态窗口默认的敲回车事件，这个多个地方用处一致
				return false;
		...
		//为关联按钮绑定事件，执行当前线索（马云）与市场活动关联的操作
		//（线索与市场活动关联关系表的添加操作）
		$("#bundBtn").click(function () {

			var $xz = $("input[name=xz]:checked");//是通过name='xz'来定位checked项
			...
			//肯定选了，而且有可能是1条或者多条
				...
				var param = "clueId=${c.id}&";//这里一定注意还有activityId要传过去啊啊啊啊啊啊
		...
		$.ajax({
			...
			success : function (data) {
				...
				$.each(data,function (i,n) {
					...
					html += '<td>'+n.endDate+'</td>';
					html += '<td>'+n.owner+'</td>';
					html += '<td><a href="javascript:void(0);"  style="text-decoration: none;" onclick="unbund(\''+n.id+'\')"<!--这个id是经过处理的，不是activityId而是关联表（clue和activity的表）主键id-->><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
