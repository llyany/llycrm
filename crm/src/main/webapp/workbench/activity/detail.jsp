...
<script type="text/javascript">
	...
	$(function(){
		...
		//页面加载完毕后，展现市场活动备注列表，使用AJAX，因为数据量可能大
		showRemarkList();
...
//发出ajax请求，刷新市场活动备注列表
            ...
            $.ajax({
                ...
                success : function (data) {
                    ...
                    $.each(data,function (i,n) {
                        ...
                        //加上'e'的原因：修改备注要定位获取备注内容noteContent，这里直接在前端拿备注信息，不间接通过经过后台，所以，要区分备注的id
                        //总结！！！！！所有要操作的行，都要获取其对应的id
        ...
        function editRemark(id) {
            //取得指定的h5标签对中的内容（需要修改的备注信息）
            var noteContent = $("#e"+id).html();

            //将需要修改的备注信息填充到修改备注模态窗口的文本域中
            $("#noteContent").val(noteContent);

            //为修改备注模态窗口隐藏域中的id赋值，作为索引！！！
            $("#remarkId").val(id);

            $("#editRemarkModal").modal("show");

        }
        //为修改操作模态窗口的更新按钮绑定事件，执行备注的修改操作
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success) {
                        ...
                        //找到删除记录对应的div，将该div移除掉
                        //我们已经为div起好了名子，名字就是当前备注的id
                        $("#" + id).remove();//"#"+id是动态BOM定位
			...
			$.ajax({
				...
				success : function (data) {
					...
					/*
						...
						data
							{"success":true/false,"ar":{备注}}
							//没有单独的模块来显示备注，而是采用追加的模式，因此，要返回通过对象来显示信息到追加的备注中
        ...
        //修改（编辑）按钮绑定事件，点击后，先把信息铺到模态窗口，然后显示编辑模态窗口
...
<body>
	...
	<!-- 大标题 -->
    <%--下面的信息是在request共享域中取得的，使用的是请求转发(jsp)，注意，在这个.jsp页面加载时已经完成了--%>
